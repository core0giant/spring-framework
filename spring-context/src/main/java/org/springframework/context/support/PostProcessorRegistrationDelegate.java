...
final class PostProcessorRegistrationDelegate {
	...
	public static void invokeBeanFactoryPostProcessors(
		...
		// 具体来说，我们不能导致处理器被实例化（通过 getBean() 调用）或
		// 以错误的顺序在 ApplicationContext 中注册。
		//
		//
		...
		if (beanFactory instanceof BeanDefinitionRegistry) {
			...
			// 我们需要保持所有常规 bean 不初始化以让 bean 工厂后处理器应用于它们！
			// 将分开实现 PriorityOrdered、Ordered
	...
	public static void registerBeanPostProcessors(
		...
		// 但使用多个列表和多次遍历处理器名称是有意的。
		// 我们必须确保遵守 PriorityOrdered 和 Ordered 处理器的合同。
		// 具体来说，我们不能导致处理器被实例化（通过 getBean() 调用）或以错误的顺序
		...
		// 注册 BeanPostProcessorChecker，它会在 BeanPostProcessor
		// 实例化期间创建 bean 时记录信息消息，
		// 即当 bean 没有资格被所有 BeanPostProcessor 处理时。
		int beanProcessorTargetCount = beanFactory.getBeanPostProcessorCount() + 1 + postProcessorNames.length;
		beanFactory.addBeanPostProcessor(new BeanPostProcessorChecker(beanFactory, beanProcessorTargetCount));

		// 分离实现 PriorityOrdered、Ordered
		// 和其余部分的 BeanPostProcessor.
